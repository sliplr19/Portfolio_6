...
NCAA_data <- NCAA_data %>%
  mutate(
    off_sos_rank =  rank((off_coeff*rpi))
  )
```
...
NCAA_data <- NCAA_data %>%
  mutate(
    def_sos_rank =  rank((def_coeff*rpi))
  )
```

Okay, cool. Now we have all the data we need. Now I'm going to write a function to compare teams.

```{r compare_func}
compare_teams <- function(dataframe_data, dataframe_bracket, round, team1, team2, offense, defense) {
  offense_1 <- dataframe['team1', 'offense']
  offense_2 <- dataframe['team2', 'offense']
  defense_1 <- dataframe['team1', 'defense']
  defense_2 <- dataframe['team2', 'defense']
  compare_1 <- offense_1 - defense_2
  compare_2 <- defense_1 - offense_2
  if(compare_1 + compare_2 > 0){
    column <- paste0("Round_", round, collapse)
    dataframe_bracket <- dataframe_bracket %>%  
      add_row(column = team1)
      }
   else{
     column <- paste0("Round_", round, collapse)
    dataframe_bracket <- dataframe_bracket %>%  
      add_row(column = team2)
      }
}
```
