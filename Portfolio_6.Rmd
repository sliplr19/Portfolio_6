...
for(i in 1:nrow(basic)){
    ...
    basic[i,1] <- str_replace(basic[i,1], "UC-Santa Barbara", "UCSB")
    ...
    basic[i,1] <- str_replace(basic[i,1], "Southern California", "USC")
}
for(i in 1:nrow(adv)){
    adv[i,1] <- str_replace(adv[i,1], "NCAA", "")
    adv[i,1] <- str_replace(adv[i,1], "State", "St.")
    adv[i,1] <- str_replace(adv[i,1], "UC-Santa Barbara", "UCSB")
    ...
    adv[i,1] <- str_replace(adv[i,1], "Southern California", "USC")
}

```

       for(i in 1:nrow(bracket)) {
  for(j in 1:nrow(basic)){
    x <- agrep(bracket[i,1], basic[j,1], max.distance = 0.1,  
                costs = NULL, value = TRUE,
...
x <- agrep(bracket[i,1], basic[j,1], max.distance = 0.1,          ignore.case = FALSE, value = TRUE)



 x <- agrep(bracket[i,1], basic$team[j,], max.distance = 0.1, costs = NULL,
      ignore.case = FALSE, value = FALSE, fixed = TRUE,
      useBytes = FALSE) 
       x <- bracket[as.logical(adist(basic[j,1],bracket) <= 2)]